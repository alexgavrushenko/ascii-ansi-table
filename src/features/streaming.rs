use crate::types::{StreamConfig, StreamUserConfig, ColumnConfig, TableError, TableResult};
use crate::core::calculator::calculate_output_column_widths;
...
impl TableStream {
    ...
    pub fn initialize_columns(&mut self, column_count: usize) {
        if self.config.columns.is_empty() {
            let mut default_column = ColumnConfig::default();
            
            default_column.width = 10; 
            self.config.columns = vec![default_column; column_count];
        }
        
        
        let dummy_data = vec![vec![String::new(); column_count]];
        self.column_widths = calculate_output_column_widths(&dummy_data, &self.config.clone().into());
    }
    ...
    pub fn write_row(&mut self, row: &[String]) -> TableResult<String> {
        ...
        if self.first_row {
            if (self.config.draw_horizontal_line)(0, 1) {
                result.push_str(&draw_border_line(&self.column_widths, &self.config.border, BorderType::Top));
                result.push('\n');
            }
            self.first_row = false;
        } else {
            
            if (self.config.draw_horizontal_line)(self.row_count, self.row_count + 1) {
                result.push_str(&draw_border_line(&self.column_widths, &self.config.border, BorderType::Join));
                result.push('\n');
            }
        }
        ...
        result.push_str(&draw_row(&processed_row, &self.column_widths, &self.config.border));
        result.push('\n');
    ...
    pub fn finalize(&mut self) -> String {
        ...
        if !self.first_row && (self.config.draw_horizontal_line)(self.row_count, self.row_count) {
            result.push_str(&draw_border_line(&self.column_widths, &self.config.border, BorderType::Bottom));
        }
    ...
    fn process_row_data(&self, row: &[String]) -> Vec<String> {
        ...
        for (col_idx, cell) in row.iter().enumerate() {
            let column_config = self.config.columns.get(col_idx).unwrap_or(&self.config.column_default);
            ...
            let content_width = target_width.saturating_sub(column_config.padding_left + column_config.padding_right);
            let aligned = self.align_cell_content(&cell_content, content_width, column_config.alignment);
            let padded = self.pad_cell_content(&aligned, column_config.padding_left, column_config.padding_right);
    ...
    fn align_cell_content(&self, content: &str, width: usize, alignment: crate::types::Alignment) -> String {
