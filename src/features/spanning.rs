use crate::types::{
    SpanningCellConfig, CellCoordinates, RangeCoordinate, RangeConfig, 
    ColumnConfig, CellConfig, Row
};
...
impl SpanningCellManager {
    ...
    pub fn get_containing_range(&self, cell: &CellCoordinates) -> Option<&RangeConfig> {
        self.ranges.iter().find(|range| {
            cell.col >= range.top_left.col && cell.col <= range.bottom_right.col &&
            cell.row >= range.top_left.row && cell.row <= range.bottom_right.row
        })
    }
    ...
    pub fn in_same_range(&self, cell1: &CellCoordinates, cell2: &CellCoordinates) -> bool {
        if let Some(range1) = self.get_containing_range(cell1) {
            if let Some(range2) = self.get_containing_range(cell2) {
                return range1.top_left == range2.top_left && range1.bottom_right == range2.bottom_right;
            }
        }
        false
    }
    ...
    pub fn get_span_origin(&self, cell: &CellCoordinates) -> Option<CellCoordinates> {
        self.get_containing_range(cell).map(|range| range.top_left.clone())
    }
    ...
    pub fn extract_spanning_cell_content(&self, cell: &CellCoordinates, data: &[Row]) -> Option<String> {
        if let Some(range) = self.get_containing_range(cell) {
...
fn create_range_config(config: &SpanningCellConfig, columns: &[ColumnConfig]) -> RangeConfig {
    ...
    let cell_config = CellConfig {
        ...
        vertical_alignment: config.vertical_alignment.unwrap_or(default_column.vertical_alignment),
...
mod tests {
    use super::*;
    use crate::types::{SpanningCellConfig, ColumnConfig};
