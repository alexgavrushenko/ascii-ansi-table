...
impl AlignmentProcessor {
    ...
    pub fn align_text_vertical(lines: &[String], height: usize, alignment: VerticalAlignment) -> Vec<String> {
        if lines.len() >= height {
            return lines.to_vec();
        }
...
mod tests {
    ...
    fn test_align_text_vertical() {
        ...
        let top_aligned = AlignmentProcessor::align_text_vertical(&lines, 5, VerticalAlignment::Top);
        assert_eq!(top_aligned, vec!["line1", "line2", "", "", ""]);
        
        let bottom_aligned = AlignmentProcessor::align_text_vertical(&lines, 5, VerticalAlignment::Bottom);
        assert_eq!(bottom_aligned, vec!["", "", "", "line1", "line2"]);
        
        let middle_aligned = AlignmentProcessor::align_text_vertical(&lines, 5, VerticalAlignment::Middle);
