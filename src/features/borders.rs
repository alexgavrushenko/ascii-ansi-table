...
impl BorderRenderer {
    ...
    pub fn draw_top_border(&self) -> String {
        self.draw_horizontal_border(&self.config.top_left, &self.config.top_right, &self.config.top_body, &self.config.top_join)
    }

    pub fn draw_bottom_border(&self) -> String {
        self.draw_horizontal_border(&self.config.bottom_left, &self.config.bottom_right, &self.config.bottom_body, &self.config.bottom_join)
    }

    pub fn draw_separator(&self) -> String {
        self.draw_horizontal_border(&self.config.join_left, &self.config.join_right, &self.config.join_body, &self.config.join_join)
    }
