...
mod tests {
    ...
    fn test_ansi_sequences_in_wrapped_cells() {
        ...
        assert!(
            ...
            "Should contain red color start sequence"
        );
        assert!(
            result.contains("\u{1b}[0m"),
    ...
    fn test_ansi_sequences_wrap_preservation() {
        ...
        assert!(
            full_content.contains("\u{1b}[31m"),
            "Should preserve red color start"
        );
        assert!(
            full_content.contains("\u{1b}[0m"),
            "Should preserve reset sequence"
        );
    }
    ...
    fn test_ansi_sequence_continuation() {
        let data = vec![vec![
            "\u{1b}[31mThis is a very long red text that should be wrapped\u{1b}[0m".to_string(),
        ]];
    ...
    fn test_complex_ansi_lorem_ipsum_table() {
        ...
        assert!(
            result.contains("\u{1b}[1;4m"),
            "Should contain bold+underline sequence"
        );
        ...
        assert!(
            result.contains("\u{1b}[35m"),
            "Should contain magenta color"
        );
        assert!(
            result.contains("\u{1b}[37;41m"),
            "Should contain white on red background"
        );
        assert!(
            result.contains("\u{1b}[30;47m"),
            "Should contain black on white background"
        );
        assert!(
            result.contains("\u{1b}[0m"),
        ...
        assert!(
            ...
            "Should contain header 'Description'"
        );
        ...
        assert!(
            result.contains("dolor sit") || result.contains("dolor"),
            "Should contain 'dolor' (may be wrapped)"
        );
        assert!(
            result.contains("amet"),
            "Should contain 'amet' (may be wrapped)"
        );
        assert!(
            result.contains("consectetur"),
            "Should contain 'consectetur'"
        );
        ...
        assert!(
            lines.len() > 15,
            "Should have many lines due to text wrapping"
        );
        ...
        for (i, line) in content_lines.iter().enumerate() {
            ...
            if parts.len() >= 5 {
                for j in 1..=4 {
                    ...
                    assert!(
                        display_width <= expected_widths[j - 1],
    ...
    fn test_vertical_alignment_with_bottom_padding() {
        let data = vec![
            ...
            vec![
                "A".to_string(),
                "Very long content that will wrap to multiple lines in this cell".to_string(),
            ],
        ];
        ...
        assert!(
            ...
            "Should contain 'Medium' (may be wrapped)"
        );
        ...
        assert!(
            lines.len() > 5,
            "Should have multiple lines due to wrapping and alignment"
        );
    ...
    fn test_bottom_padding_adds_newlines() {
        ...
        use crate::types::{ColumnConfig, TableConfig, VerticalAlignment};

        let _row_data = vec![vec!["Line1".to_string()]];

        let wrapped_data = vec![vec![vec!["Line1".to_string()]]];
    ...
    fn test_middle_padding_adds_newlines() {
        ...
        assert_eq!(
            result[0],
            vec![""],
            "First row should be empty (top padding)"
        );
        assert_eq!(
            result[1],
            vec![""],
            "Second row should be empty (top padding)"
        );
        assert_eq!(result[2], vec!["Line1"], "Third row should have content");
        assert_eq!(
            result[3],
            vec![""],
            "Fourth row should be empty (bottom padding)"
        );
        assert_eq!(
            result[4],
            vec![""],
            "Fifth row should be empty (bottom padding)"
        );
    }
    ...
    fn test_table_with_header() {
        ...
        assert!(
            result.contains("═"),
            "Header separator '═' not found in output"
        );
    ...
    fn test_table_with_header_different_styles() {
        ...
        let body_lines: Vec<&str> = lines
            .iter()
            .filter(|line| {
                line.contains("Laptop") || line.contains("Mouse") || line.contains("Keyboard")
            })
            .cloned()
            .collect();
        ...
        for line in body_lines {
            assert!(
                line.contains("│"),
                "Body lines should use regular vertical separators"
            );
        }
    }
    ...
    fn test_table_with_header_column_configuration() {
        ...
        assert!(
            result.contains("╕"),
            "Header top-right corner '╕' not found"
        );
        ...
        let header_row = lines
            .iter()
            .find(|line| line.contains("Short") && line.contains("Very Long Header Name"));
    ...
    fn test_table_with_header_no_body_rows() {
        ...
        assert!(
            lines.len() >= 3,
            "Should have at least 3 lines (top border, header row, bottom border)"
        );

        let header_sep_lines: Vec<&str> = lines
            .iter()
            .filter(|line| line.contains("╠") && line.contains("╣") && line.contains("═"))
            .cloned()
            .collect();
        assert!(
            header_sep_lines.is_empty(),
            "Should not have header separator when there are no body rows"
        );
    }
    ...
    fn test_table_without_header_configuration() {
        ...
        assert!(
            result.contains("│"),
            "Regular vertical separators not found"
        );
        assert!(result.contains("├"), "Regular row separators not found");
        assert!(result.contains("└"), "Regular bottom-left corner not found");
        assert!(
            result.contains("┘"),
            "Regular bottom-right corner not found"
        );

        assert!(
            !result.contains("╔"),
            "Should not contain header top-left corner"
        );
        assert!(
            !result.contains("╗"),
            "Should not contain header top-right corner"
        );
        assert!(
            !result.contains("║"),
            "Should not contain header vertical separators"
        );
        assert!(
            !result.contains("╠"),
            "Should not contain header row separator left"
        );
        assert!(
            !result.contains("╣"),
            "Should not contain header row separator right"
        );
        assert!(
            !result.contains("═"),
            "Should not contain header horizontal lines"
        );
        ...
        for line in content_lines {
            assert!(
                line.contains("│"),
                "All content lines should use regular vertical separators"
            );
        }
    }
    ...
    fn test_inconsistent_column_widths_bug() {
        let data = vec![
            ...
            vec![
                "Row 2 Col 1 -".to_string(),
                "Row 2 Col 2 --".to_string(),
                "Row 2 Col 3 ---".to_string(),
            ],
        ];
        ...
        let border_lines: Vec<&str> = lines
            .iter()
        ...
        assert!(
            result.contains("║"),
            "Should contain header vertical borders"
        );
    }
    ...
    fn test_processor_functions_integration() {
        let data = vec![
            vec![
                ...
                "Center".to_string(),
                "Right".to_string(),
                "VeryLongTextThatNeedsTruncation".to_string(),
            ],
            vec![
                "A".to_string(),
                "B".to_string(),
                "C".to_string(),
                "AnotherLongTextExample".to_string(),
            ],
        ];
        ...
        assert!(
            lines.len() >= 5,
            "Should have at least 5 lines (borders + content)"
        );
        ...
        assert_eq!(
            column_count, 5,
            "Should have 5 vertical separators (4 columns + borders)"
        );
            ...
            .filter(|line| line.contains("│") && !line.contains("─"))
        {
            ...
            if segments.len() >= 5 {
                assert_eq!(
                    segments[1].len(),
                    8,
                    "First column should be 8 characters wide"
                );
                assert_eq!(
                    segments[2].len(),
                    10,
                    "Second column should be 10 characters wide"
                );
                assert_eq!(
                    segments[3].len(),
                    8,
                    "Third column should be 8 characters wide"
                );
                assert_eq!(
                    segments[4].len(),
                    12,
                    "Fourth column should be 12 characters wide"
                );
            }
        }
