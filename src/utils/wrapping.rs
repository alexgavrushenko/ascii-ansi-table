...
pub fn wrap_text(text: &str, width: usize, word_wrap: bool) -> Vec<String> {
    ...
    let mut result: Vec<String> = wrapped_lines.into_iter().map(|line| line.to_string()).collect();
...
mod tests {
    ...
    fn test_wrap_text_by_words() {
        ...
        assert_eq!(result, vec!["This is a", "long line", "that", "should be", "wrapped"]);
    }
    ...
    fn test_ansi_wrapping_fix() {
        ...
        for (i, line) in result.iter().enumerate() {
            if i < result.len() - 1 {
                assert!(line.ends_with("\u{1b}[0m"), "Line {} should end with reset: '{}'", i, line);
            }
        }
        ...
        for (i, line) in result.iter().enumerate() {
            if i > 0 {
                assert!(line.starts_with("\u{1b}[31m"), "Line {} should start with red color: '{}'", i, line);
            }
        }
    }
    ...
    fn test_multiple_ansi_sequences() {
        ...
        for (i, line) in result.iter().enumerate() {
            if i > 0 {
                assert!(line.starts_with("\u{1b}[31m\u{1b}[1m"), "Line {} should start with red+bold: '{}'", i, line);
            }
        }
    }
