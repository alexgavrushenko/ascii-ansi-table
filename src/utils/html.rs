...
pub fn convert_ansi_to_html(text: &str) -> String {
    ...
    for (code, color) in fg_colors {
        let pattern = format!(r"\x1b\[{}m", code);
    ...
    result = result.replace("\x1b[9m", r#"<span style="text-decoration: line-through;">"#);
    ...
    result = re_256_fg.replace_all(&result, |caps: &regex::Captures| {
    ...
    result = re_256_bg.replace_all(&result, |caps: &regex::Captures| {
    ...
    result = re_rgb_fg.replace_all(&result, |caps: &regex::Captures| {
    ...
    result = re_rgb_bg.replace_all(&result, |caps: &regex::Captures| {
    ...
    result = re_cleanup.replace_all(&result, "</span>").to_string();
    
    
    result = format!("<pre style=\"margin: 0; font-family: monospace; white-space: pre;\">{}</pre>", result);
...
mod tests {
    ...
    fn test_basic_color_conversion() {
        ...
        let expected = r#"<span style="color: #ff0000;">Red text</span>"#;
        assert_eq!(convert_ansi_to_html(input), expected);
    }
    
    #[test]
    fn test_bold_formatting() {
        let input = "\x1b[1mBold text\x1b[0m";
        let expected = r#"<span style="font-weight: bold;">Bold text</span>"#;
        assert_eq!(convert_ansi_to_html(input), expected);
    }
    ...
    fn test_rgb_color() {
        ...
        let expected = r#"<span style="color: rgb(255, 128, 0);">Orange text</span>"#;
        assert_eq!(convert_ansi_to_html(input), expected);
    }
