...
use crate::utils::{calculate_maximum_column_widths, calculate_cell_height, wrap_cell};
...
pub fn calculate_row_heights(rows: &[Row], config: &TableConfig) -> Vec<usize> {
    ...
    for row in rows {
        ...
        for (col_idx, cell) in row.iter().enumerate() {
            ...
            let content_width = cell_width.saturating_sub(column_config.padding_left + column_config.padding_right);
...
pub fn map_data_using_row_heights(rows: &[Row], row_heights: &[usize], config: &TableConfig) -> Vec<Vec<Row>> {
    let column_widths = calculate_output_column_widths(rows, config);
    ...
    for (row_idx, row) in rows.iter().enumerate() {
        ...
        for (col_idx, cell) in row.iter().enumerate() {
            ...
            for (line_idx, mapped_row) in mapped_rows.iter_mut().enumerate() {
                let line_content = wrapped_lines.get(line_idx).unwrap_or(&String::new()).clone();
