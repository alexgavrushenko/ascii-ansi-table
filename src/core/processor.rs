...
pub fn align_table_data_with_widths(
    rows: &[Row],
    config: &TableConfig,
    column_widths: &[usize],
) -> Vec<Row> {
...
pub fn pad_table_data_with_widths(
    rows: &[Row],
    config: &TableConfig,
    column_widths: &[usize],
) -> Vec<Row> {
...
pub fn apply_vertical_alignment(
    ...
    for (row_idx, row_group) in rows.iter().enumerate() {
        ...
        for col_idx in 0..column_count {
            ...
            let column_data: Vec<String> = row_group
                .iter()
                .map(|r| r.get(col_idx).unwrap_or(&String::new()).clone())
                .collect();

            let aligned_column = align_column_vertically(
                &column_data,
                *target_height,
                column_config.vertical_alignment,
            );
