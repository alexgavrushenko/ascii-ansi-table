use crate::types::{Row, TableConfig, BorderConfig};
use crate::core::calculator::{calculate_output_column_widths, calculate_row_heights, map_data_using_row_heights};
use crate::core::processor::{align_table_data_with_widths, pad_table_data_with_widths, truncate_table_data};
...
pub fn draw_table(rows: &[Row], config: &TableConfig) -> String {
    ...
    if (config.draw_horizontal_line)(0, rows.len()) {
        let border_config = if has_header { &header_config.border } else { &config.border };
        result.push_str(&draw_border_line(&column_widths, border_config, BorderType::Top));
        result.push('\n');
    }
    ...
    for (row_idx, row_group) in processed_data.iter().enumerate() {
        ...
        for (_sub_row_idx, sub_row) in row_group.iter().enumerate() {
        ...
        if row_idx < processed_data.len() - 1 {
            if is_header_row {
                ...
                result.push_str(&draw_border_line(&column_widths, &header_config.border, BorderType::Header));
                result.push('\n');
            } else if (config.draw_horizontal_line)(row_idx + 1, rows.len()) {
                result.push_str(&draw_border_line(&column_widths, &config.border, BorderType::Join));
                result.push('\n');
            }
        }
    }
    ...
    if (config.draw_horizontal_line)(rows.len(), rows.len()) {
        result.push_str(&draw_border_line(&column_widths, &config.border, BorderType::Bottom));
    }
...
fn process_mapped_data(mapped_data: &[Vec<Vec<String>>], config: &TableConfig, column_widths: &[usize]) -> Vec<Vec<Vec<String>>> {
    ...
    for row_group in mapped_data {
        ...
        for sub_row in row_group {
            ...
            let aligned_row = align_table_data_with_widths(&[sub_row.clone()], config, column_widths);
...
pub fn draw_border_line(column_widths: &[usize], border: &BorderConfig, border_type: BorderType) -> String {
    let mut result = String::new();
    
    let (left, right, body, join) = match border_type {
        BorderType::Top => (&border.top_left, &border.top_right, &border.top_body, &border.top_join),
        BorderType::Bottom => (&border.bottom_left, &border.bottom_right, &border.bottom_body, &border.bottom_join),
        BorderType::Join => (&border.join_left, &border.join_right, &border.join_body, &border.join_join),
        BorderType::Header => (&border.join_left, &border.join_right, &border.header_join, &border.join_join),
    };
